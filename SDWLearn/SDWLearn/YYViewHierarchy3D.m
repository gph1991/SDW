...
@interface  YYViewHierarchy3DButton : UIWindow

+ (YYViewHierarchy3DButton *)sharedInstance;

@end

@implementation YYViewHierarchy3DButton

+ (YYViewHierarchy3DButton *)sharedInstance
{
    static dispatch_once_t once;
    static YYViewHierarchy3DButton *singleton;
    dispatch_once(&once, ^{
        singleton = [[YYViewHierarchy3DButton alloc] init];
    });
...
- (void)pan:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    NSLog(@"tran:%@",NSStringFromCGPoint([gestureRecognizer translationInView:self]));
    NSLog(@"loca:%@",NSStringFromCGPoint([gestureRecognizer locationInView:self]));
...
+ (void)show
{
    [YYViewHierarchy3DButton sharedInstance].hidden = NO;
    [YYViewHierarchy3D sharedInstance].hidden = YES;
}

+ (void)hide
{
    [YYViewHierarchy3DButton sharedInstance].hidden = YES;
    [YYViewHierarchy3D sharedInstance].hidden = YES;
}
